...
namespace Web.Controllers
{
    public class WebApiController : ApiController
    {
        ...
        public IHttpActionResult GetIndex()
        {
            ...
            //                             select product).ToList();
            //蟹塘采购公蟹列表
            List<CsProducts> productF = products.Where(x => x.ProductType == 2 && x.ProductName.StartsWith("公")).OrderBy(x=>x.ProductWeight).ToList();
            //蟹塘采购母蟹列表
            List<CsProducts> productM = products.Where(x => x.ProductType == 2 && x.ProductName.StartsWith("母")).OrderBy(x=>x.ProductWeight).ToList();
            var numberCount = productF.Count > productM.Count?productF.Count:productM.Count;
            ...
            for (int j = 0; j < numberCount; j++)
            {
                ...
                if (j < productF.Count)
                {
                    strJson.Append("\"pn1\":\""+productF[j].ProductName+"\",\"pv1\":"+(productF[j].ProductState==1?productF[j].ProductPrice.ToString():"\"-\"")+",");
                }
                else
                {
                    strJson.Append("\"pn1\":\"\",\"pv1\":\"\",");
                }

                if (j < productM.Count)
                {
                    strJson.Append("\"pn2\":\"" + productM[j].ProductName + "\",\"pv2\":" + (productM[j].ProductState == 1 ? productM[j].ProductPrice.ToString() : "\"-\""));
                }
                else
                {
                    strJson.Append("\"pn2\":\"\",\"pv2\":\"\"");
                }
                strJson.Append("}");
                if (j != (numberCount - 1))
                {
        ...
        public IHttpActionResult GetProductList()
        {
            ...
            List<CsProducts> proList1 = (from product1 in productList
                                         ...
                                         orderby product1.ProductWeight 
